...
function installQuestions() {
	...
	if [[ $CUSTOMIZE_ENC == "n" ]]; then
		...
		CC_CIPHER="TLS-ECDHE-ECDSA-WITH-AES-128-GCM-SHA1"
		DH_TYPE="1" # ECDH
		DH_CURVE="prime256v1"
		HMAC_ALG="SHA1"
		TLS_SIG="1" # tls-crypt
		...
		case $CERT_TYPE in
		1)
			echo "   1) ECDHE-ECDSA-AES-128-GCM-SHA1 (recommended)"
			...
			;;
		2)
			echo "   1) ECDHE-RSA-AES-128-GCM-SHA1 (recommended)"
			...
			case $CC_CIPHER_CHOICE in
			1)
				CC_CIPHER="TLS-ECDHE-RSA-WITH-AES-128-GCM-SHA1"
				;;
			2)
